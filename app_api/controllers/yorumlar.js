...
const yorumGetir=function(req, res) {
	//yorumid parametresi var mı kontrol edilir
	if(req.params && req.params.mekanid && req.params.yorumid){
		Mekan.findById(req.params.mekanid)
		//sadece mekan adını ve mekan yorumlarını getir
		.select('ad yorumlar')
		.exec(function(hata, mekan) {
			...
			if(mekan.yorumlar && mekan.yorumlar.length > 0){
				...
				}//varsa cevap nesnesi döndür. İçine mekan adı, idsi ve yorumu ekle
				else {
					cevap= {
						mekan : {
							ad : mekan.ad,
